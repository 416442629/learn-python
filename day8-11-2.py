...
class FrenchDeck:
    ...
    #print(ranks)
    suits='spaces diamonds clubs heart'.split()
    print(suits)
    def __init__(self):
...
def spades_high(card):
    #定义在累外部
    suit_values = dict(spaces=3, heart=2, diamonds=1, clubs=0)
    #print(len(suit_values))
     # suit_values['spaces']=6  可以改变值
     # print('dict:',suit_values)
    rank_value =FrenchDeck.ranks.index(card.rank)
    return rank_value*len(suit_values)+suit_values[card.suit]
#len(suit_values)的值为dict key的个数


deck = FrenchDeck()
cad2=Card('5', 'spaces')
print('看值',spades_high(cad2))
